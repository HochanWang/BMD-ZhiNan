...
这门课是在第四学期之后的两个星期进行，需要提前很久发邮件报名注册才有名额（但基本报不满）。除非对这个课题或自行车运动非常感兴趣，否则不建议选择，有这个时间不如准备可能的补考或者开始暑假，这门课的设计比较失败，感觉学不到什么。

这门课分为上课和做Project,上课的大部分内容并没有实际作用，都是非常粗浅的介绍，而且很多内容与BMD的其他课程内容有一定重复。Project的内容为预测测试者在赛道上的表现（特定场景下完成指定距离的时间等）并在最后一天进行实地测试并比较。
\begin{flushright}
张先治; 15/07/2023
\end{flushright}
